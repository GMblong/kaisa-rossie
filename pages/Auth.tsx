...
import { 
  ...
  UserCheck, Briefcase, DollarSign, Bus, BadgeCheck, ArrowLeft,
  Target, Ticket
} from 'lucide-react';
...
export const Auth: React.FC<AuthProps> = ({ onLogin, onNavigate }) => {
  ...
  const roleOptions = [
    { role: UserRole.CUSTOMER, label: 'Customer', icon: UserCheck, desc: 'Akses Perjalanan Saya' },
    { role: UserRole.SUPER_ADMIN, label: 'Super Admin', icon: ShieldCheck, desc: 'Kontrol Sistem Penuh' },
    { role: UserRole.SALES_AGENT, label: 'Sales/Agent', icon: Target, desc: 'Pipeline & Komisi' },
    { role: UserRole.TICKETING, label: 'Ticketing', icon: Ticket, desc: 'Pesawat & Visa' },
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center p-8 md:p-12 lg:p-20 bg-white relative">
        <div className="w-full max-w-2xl space-y-8 lg:space-y-12 overflow-y-auto max-h-[90vh] no-scrollbar">
          <div className="space-y-4 text-center lg:text-left">
            <h1 className="text-4xl lg:text-5xl font-black text-emerald-950 tracking-tighter">Private Portal</h1>
            <p className="text-emerald-950/40 font-bold uppercase text-[10px] lg:text-[11px] tracking-[0.3em]">Pilih gerbang akses masuk Anda</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4">
            {roleOptions.map((opt) => (
              <button 
                ...
                className={`group flex items-center space-x-4 p-4 lg:p-5 rounded-2xl lg:rounded-3xl transition-all text-left border ${
              ...
              >
                ...
                <div>
                  <p className="text-xs lg:text-sm font-black">{opt.label}</p>
                  <p className={`text-[8px] lg:text-[9px] font-bold uppercase tracking-widest ${opt.role === UserRole.SUPER_ADMIN ? 'text-white/40' : 'text-emerald-950/30'}`}>{opt.desc}</p>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="space-y-4">
             <input type="email" className="w-full bg-emerald-950/5 border border-emerald-950/10 rounded-2xl py-4 px-8 font-bold text-emerald-950 outline-none focus:border-gold-500 transition-all text-sm" placeholder="Email Address" />
             <input type="password" className="w-full bg-emerald-950/5 border border-emerald-950/10 rounded-2xl py-4 px-8 font-bold text-emerald-950 outline-none focus:border-gold-500 transition-all text-sm" placeholder="Password" />
             <button className="w-full bg-emerald-950 text-white py-5 rounded-2xl font-black text-[10px] lg:text-xs uppercase tracking-[0.3em] hover:bg-gold-500 transition-all shadow-xl">Submit Access</button>
          </div>
        </div>
      </div>
    </div>
